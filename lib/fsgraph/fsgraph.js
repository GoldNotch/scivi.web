...
if (IN_VISUALIZATION)
{
    ...
    if (!graph){
        ...
        console.log('parsed graph', Date.now() / 1000);
	...
	"links_tab": new FSGraph.LinksTab(),
    ...
    if (!gui) {
        ...
        console.log('start build gui', Date.now()/ 1000);
        gui.build(container, gui_configuration, translator);
        console.log('finish build gui', Date.now()/ 1000);
        if (states_count > 0)
            gui.bindGraph(graph, graph_view_configuration);
        console.log('bound graph', Date.now()/ 1000);
    }
